...
import ControlPlane from "../../../../src/assets/images/learn-khulnasoft/linkerd/control-plane-c.webp";
...
Use meshplay to deploy the bookinfo application :

- In Meshplay, navigate to the Linkerd adapter's management page from the left nav menu.
...
- Download the [./sample/bookinfo.yaml](https://raw.githubusercontent.com/khulnasoft/linkerd-service-mesh-workshop/master/lab-7/sample/book-info.yaml)
...
- Remove the bookinfo application from the `Meshplay Dashboard` by clicking on the `trash icon` in the `sample application` card on the linkerd adapters' page.
