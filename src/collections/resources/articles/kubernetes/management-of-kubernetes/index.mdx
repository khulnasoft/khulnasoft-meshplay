...
import picture1 from "./meshplay-core-architecture.webp"; 
import picture2 from "./settings.webp"; 
import picture3 from "./context-switcher.webp"; 
import ClusterImg from "./multi-cluster-kubernetes-management-with-meshplay.webp";
...
<ResourcesWrapper>
  ...
  <p> Kubernetes management is the process of managing your containers on a Kubernetes cluster. This can include things like adding or removing clusters, scaling 
    ...
    involve many different types of actions. Figuring out how to do them all manually would be extremely time-consuming. Fortunately, there are tools like Meshplay 
...
<CTA_FullWidth 
  image={ClusterImg}
  alt="Multi-Cluster Kubernetes Management with Meshplay"
  content="Multi-Cluster Kubernetes Management with Meshplay"
  button_text="Read blog post"
  url="/blog/meshplay/multi-cluster-kubernetes-management-with-meshplay"
  external_link={false}
...
/>
  ...
  <h3> How Meshplay makes it easier to run Kubernetes </h3>
  
  <p> Meshplay is the only cloud-native manager in the world that supports more adapters than any other project or product. </p>
  
  <img src = {picture1} class="image-center" alt="Management of Kubernetes with Meshplay" />
  
  <p> Meshplay has been designed for the world of many service meshes and many Kubernetes clusters. As such, great attention was made to guarantee that it is an 
  extensible management platform, able to handle a diverse range of infrastructure and new use cases quickly through its plugin mechanism. Meshplay Server acts as an 
  operation delegator, determining which Meshplay Adapter has registered its capacity for the given operation. The operation is then sent to the appropriate component 
  using a gRPC call. This could be one of Meshplay's service mesh adapters, like the Istio adapter. </p> 
  
  <p> Meshplay's capability is constantly expanding, from multi-mesh to now multi-cluster, to give developers, operators, and security engineers more control over 
  their infrastructure. Each part of Meshplay's architecture makes a big difference in how it manages multiple Kubernetes clusters. </p>
  
  <h3> Meshplay management across many clusters </h3>
  ...
  <img src = {picture2} class="image-center" alt="Management of Kubernetes with Meshplay" />
  
  <p> Meshplay also deploys Meshplay operators throughout the cluster it is about to manage. This operator is in charge of the Meshplay broker and the MeshSync 
  ...
  NATS server, of which the Meshplay server is a client. Meshplay server then receives all necessary data relating to cluster activity. </p>
  
  <img src = {picture3} class="image-center" alt="Management of Kubernetes with Meshplay" />
  
  <p> Meshplay, by default, wants to be as aware of your infrastructure as possible in order to deliver value. As such, it deploys its operator across each identified 
