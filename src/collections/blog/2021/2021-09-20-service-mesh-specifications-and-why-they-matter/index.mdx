...
Learn what makes each of them unique and why they are much needed. See each of these three specifications in action as we use Meshplay, the open-source service mesh management plane to demonstrate the value and functionality of each service mesh abstraction, and the adherence of these specifications by Istio, Linkerd, Consul and other popular service meshes.
...
<ul>
    ...
    <li> As a community, we turned to Meshplay as the tool to implement <Link to="/projects/service-mesh-interface-conformance">SMI conformance</Link> and we have been working with the individual service meshes to validate their conformance. </li>
</ul>    
<b>Meshplay</b>
<ul>
  <li> We work on an open-source project called <Link to="/cloud-native-management/meshplay">Meshplay</Link>.</li>
  <li> Meshplay, the cloud native management plane, is the canonical implementation of the service mesh performance.</li>
...
Let's take a moment to demo what it looks like to validate conformance in SMI using Meshplay.

<img src={Graph} className="slides-left" align="left" alt="graph" />
<ul>
   <li> We need to spin up Meshplay locally</li>
   <li> We use meshplayctl as the command line interface to work with Meshplay.</li>
   ...
   <li>These tests go and do assertions across these different specifications. We’re looking at traffic access, traffic splitting, traffic specification. Meshplay then collects these results and will eventually be publishing them in combination with the SMI project.</li>
</ul>
...
<ul>
  <li> On the terminal, we have a local deployment of Meshplay running. You can also deploy on Kubernetes as well as the vendor Kubernetes platforms like AKS, EKS and GCP or you can use a dockerized container to run Meshplay. You can also have your Kubernetes on Docker desktop.</li>
  <li>We have the Open service mesh deployed.</li>
  <li> The Meshplay UI is exposed at 9081 port. This is the UI which is used to instantiate a Load test.</li>
  ...
  <li> You can also see that Meshplay has the capability to search your environment, see what specifications are being used and what's the load on your Kubernetes.</li>
...
_**P.S.: If these topics excite  come and say "Hi" on our [Slack Channel](http://slack.khulnasoft.com) and one of us will reach out to you!**_
