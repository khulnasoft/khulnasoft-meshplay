...
const CatalogGrid = ({ frontmatter }) => {
  ...
  useEffect(() => {
    const CLOUD_FETCH_DESIGN = `https://meshplay.khulnasoft.com/api/catalog/content/pattern?technology=${
        ...
        <SistentThemeProviderWithoutBaseLine initialMode={isDark ? "dark" : "light"}>
          {designs?.patterns?.map((item, index) => {
            return (
              <CatalogCard
                key={index}
                cardLink={`https://meshplay.khulnasoft.com/catalog/content/catalog/${item?.id}`}
                cardHeight="20rem"
                cardWidth="16rem"
                pattern={item}
                type="Catalog"
